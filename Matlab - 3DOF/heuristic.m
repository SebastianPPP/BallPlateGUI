...
% =========================================================
out_dir = 'wyniki_3dof_heurystyka_dane';
if ~exist(out_dir, 'dir')
    mkdir(out_dir);
end

txt_filename = fullfile(out_dir, 'wskazniki_jakosci_heurystyka_3dof.txt');
fileID = fopen(txt_filename, 'w', 'native', 'UTF-8');
fprintf(fileID, '===================================================================\n');
fprintf(fileID, 'WYNIKI SYMULACJI - OPTYMALIZACJA HEURYSTYCZNA (3DOF) I ZAPASY STABILNOŚCI\n');
fprintf(fileID, '===================================================================\n');
...
Tsim    = 6.0;
lim     = deg2rad(15);
R_plate = 0.30;  % Promień okrągłego blatu dla 3DOF
FALL_PENALTY = 1e4;
...
% =========================================================
fprintf('=========================================================\n');
fprintf('FAZA 1: URUCHAMIANIE OPTYMALIZACJI HEURYSTYCZNEJ (3DOF)\n');
fprintf('=========================================================\n');
...
for i=1:N_p
    [J_,~,~,~,~,~,~,~] = sim_3dof(pos(i,:),A,B,x0_opt_x,x0_opt_y,dt,Tsim,lim,R_plate,FALL_PENALTY,w1,w2,w3);
    pbest_J(i) = J_;
end
...
for i=1:T_arch
    [arch_J(i),~,~,~,~,~,~,~] = sim_3dof(arch_pos(i,:),A,B,x0_opt_x,x0_opt_y,dt,Tsim,lim,R_plate,FALL_PENALTY,w1,w2,w3);
end
...
for iter = 1:N_ia
    ...
    for ant=1:N_ants
        ...
        new_pos(ant,:) = th;
        [new_J(ant),~,~,~,~,~,~,~] = sim_3dof(th,A,B,x0_opt_x,x0_opt_y,dt,Tsim,lim,R_plate,FALL_PENALTY,w1,w2,w3);
...
for i=1:N_nsga, [F1(i),F2(i)]=sim2_3dof(pop(i,:),A,B,x0_opt_x,x0_opt_y,dt,Tsim,lim,R_plate,FALL_PENALTY); end
...
for iter = 1:N_ins
    ...
    for i=1:N_nsga, [oF1(i),oF2(i)]=sim2_3dof(offspring(i,:),A,B,x0_opt_x,x0_opt_y,dt,Tsim,lim,R_plate,FALL_PENALTY); end
...
fig_bode_all = figure('Name', 'Zbiorcze charakterystyki Bode - Heurystyka 3DOF', 'Position', [100 100 900, 700]);
fig_nyq_all  = figure('Name', 'Zbiorcze wykresy Nyquista - Heurystyka 3DOF', 'Position', [150 150 900, 700]);

opt_bode = bodeoptions;
opt_bode.Grid = 'on';
opt_bode.PhaseVisible = 'on';
opt_bode.Title.String = 'Charakterystyka Bode - Optymalizacja Heurystyczna (3DOF)';
opt_bode.XLabel.String = 'Częstotliwość [rad/s]';

opt_nyq = nyquistoptions;
opt_nyq.Grid = 'on';
opt_nyq.Title.String = 'Wykres Nyquista - Optymalizacja Heurystyczna (3DOF)';
...
saveas(fig_bode_all, fullfile(out_dir, 'ZBIORCZY_Heurystyka_3DOF_Bode.png'));
close(fig_bode_all);

figure(fig_nyq_all);
legend(methods, 'Location', 'northeast', 'Interpreter', 'none');
saveas(fig_nyq_all, fullfile(out_dir, 'ZBIORCZY_Heurystyka_3DOF_Nyquist.png'));
close(fig_nyq_all);
...
%% =========================================================
fig_handles = [];
f1_h = figure('Name','Analiza Zbieżności Algorytmów 3DOF','Color','white','Position',[30 30 1400 400]);
...
f2_h = figure('Name','Historia Zmian Parametrów (Wag LQR) 3DOF','Color','white','Position',[30 480 1400 400]);
...
% =========================================================
fprintf('=========================================================\n');
fprintf('FAZA 2: TESTOWANIE REGULATORÓW W ROŻNYCH SYTUACJACH (3DOF)\n');
fprintf('=========================================================\n');
...
for s = 1:length(test_scenarios)
    ...
    f_scen = figure('Name',['Weryfikacja 3DOF - Sytuacja: ' scen.name],'Color','white','Position',[50 150 1500 450]);
    ...
    line_styles = {'-', '--', ':'};
    for m = 1:3
        [~, ~, ~, ~, hx, hy, hux, t_out, h_alpha, h_beta, fallen, fallen_t] = sim_full_3dof(thetas{m}, A, B, scen.x0_x, scen.x0_y, dt, Tsim, lim, R_plate, FALL_PENALTY, w1, w2, w3);
        d_ = sqrt(hx.^2 + hy.^2);
        ...
        writetable(sim_data, fullfile(out_dir, sprintf('heurystyka_3dof_%s_%s_dane.csv', scen.name, methods{m})));
    ...
    saveas(f_scen, fullfile(out_dir, sprintf('wykres_heurystyka_3dof_weryfikacja_%s.png', scen.name)));
end
...
%% =========================================================
fprintf('\nTrwa zapisywanie grafiki symulacyjnej 3DOF do plików...\n');
fig_names = {'wykres_heurystyka_3dof_zbieznosc', 'wykres_heurystyka_3dof_ewolucja_wag', ...
             'wykres_heurystyka_3dof_weryfikacja_Prosta', 'wykres_heurystyka_3dof_weryfikacja_Trudna'};
...
fprintf('Wszystkie pliki i raporty dla optymalizacji heurystycznej 3DOF zostały zapisane w: %s\n', out_dir);

%% =========================================================
%  FUNKCJE POMOCNICZE (3DOF)
%% =========================================================
function [J, f1, f2, Ts, hx, hy, hux, t_out] = sim_3dof(theta, ...
        A, B, x0_x, x0_y, dt, Tsim, lim, R_plate, FP, w1, w2, w3)
    q = 10.^theta(1:4);
    r = 10^theta(5);
...
function [J, f1, f2, Ts, hx, hy, hux, t_out, h_alpha, h_beta, fallen, fallen_t] = sim_full_3dof(theta, ...
        A, B, x0_x, x0_y, dt, Tsim, lim, R_plate, FP, w1, w2, w3)
    q = 10.^theta(1:4);
    r = 10^theta(5);
    [~,~,K] = care(A, B, diag(q), r);
...
function [f1,f2] = sim2_3dof(theta,A,B,x0_x,x0_y,dt,Tsim,lim,R_plate,FP)
    [~,f1,f2,~,~,~,~,~] = sim_3dof(theta,A,B,x0_x,x0_y,dt,Tsim,lim,R_plate,FP,1,0,0);
end
