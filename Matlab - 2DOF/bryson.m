...
out_dir = 'wyniki_bryson_dane';
if ~exist(out_dir, 'dir')
    mkdir(out_dir);
end

txt_filename = fullfile(out_dir, 'wskazniki_jakosci_bryson.txt');
fileID = fopen(txt_filename, 'w', 'native', 'UTF-8');
fprintf(fileID, '===================================================================\n');
fprintf(fileID, 'WYNIKI SYMULACJI - TESTY REGUŁY BRYSONA (2DOF) I ZAPASY STABILNOŚCI\n');
fprintf(fileID, '===================================================================\n');
...
fig_bode_all = figure('Name', 'Zbiorcze charakterystyki Bode - Bryson', 'Position', [100 100 900, 700]);
fig_nyq_all  = figure('Name', 'Zbiorcze wykresy Nyquista - Bryson', 'Position', [150 150 900, 700]);

opt_bode = bodeoptions;
opt_bode.Grid = 'on';
opt_bode.PhaseVisible = 'on';
opt_bode.Title.String = 'Charakterystyka Bode - Reguła Brysona';
opt_bode.XLabel.String = 'Częstotliwość [rad/s]';

opt_nyq = nyquistoptions;
opt_nyq.Grid = 'on';
opt_nyq.Title.String = 'Wykres Nyquista - Reguła Brysona';
...
saveas(fig_bode_all, fullfile(out_dir, 'ZBIORCZY_Bryson_Bode.png'));
close(fig_bode_all);

figure(fig_nyq_all);
legend(iter_labels, 'Location', 'northeast', 'Interpreter', 'none');
saveas(fig_nyq_all, fullfile(out_dir, 'ZBIORCZY_Bryson_Nyquist.png'));
close(fig_nyq_all);
...
for s = 1:length(test_scenarios)
    ...
    f = figure('Name', ['Weryfikacja Brysona - Sytuacja: ' scen.name], 'Color', 'white', 'Position', [50 150 1500 450], 'Visible', 'off');
    ...
    for k = 1:n_iter
        ...
        writetable(sim_data, fullfile(out_dir, sprintf('bryson_%s_%s_dane.csv', scen.name, name)));
    ...
    saveas(f, fullfile(out_dir, sprintf('wykres_bryson_%s.png', scen.name)));
    close(f);
end

fclose(fileID);
fprintf('Wszystkie testy reguły Brysona dla 2DOF zakończone. Dane i wykresy zapisano w folderze: %s\n', out_dir);